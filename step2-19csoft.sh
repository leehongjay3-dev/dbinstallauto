...
check_prerequisites() {
    ...
    # 检查ORACLE_HOME目录是否存在
    if [ ! -d "/app/oracle/product/19/db_1" ]; then
        log_error "ORACLE_HOME目录不存在: /app/oracle/product/19/db_1"
        return 1
    fi
    ...
    # 检查runInstaller是否存在
    if [ ! -f "/app/oracle/product/19/db_1/runInstaller" ]; then
        log_error "runInstaller不存在: /app/oracle/product/19/db_1/runInstaller"
        return 1
    fi
...
install_oracle_db() {
    ...
    # 切换到ORACLE_HOME目录
    cd "/app/oracle/product/19/db_1" || {
        log_error "无法切换到目录: /app/oracle/product/19/db_1"
        return 1
    }
    ...
    if grep -q "Execute" "$INSTALL_LOG_FILE"; then
        ...
        # 提取root.sh路径
        local root_script=$(grep -o "/app/oracle/product/19/db_1/root.sh" "$INSTALL_LOG_FILE" | head -1)
        ...
        if [ -z "$root_script" ] && [ -f "/app/oracle/product/19/db_1/root.sh" ]; then
            log_info "2. /app/oracle/product/19/db_1/root.sh"
        fi
...
execute_root_scripts() {
    ...
    if [ "$(id -u)" != "0" ]; then
        ...
        if [ -f "/app/oracle/product/19/db_1/root.sh" ]; then
            echo "2. /app/oracle/product/19/db_1/root.sh"
        fi
    ...
    # 执行root.sh
    if [ -f "/app/oracle/product/19/db_1/root.sh" ]; then
        log_info "执行: /app/oracle/product/19/db_1/root.sh"
        if /app/oracle/product/19/db_1/root.sh; then
        ...
        log_warning "未找到 /app/oracle/product/19/db_1/root.sh"
    fi
...
verify_installation() {
    ...
    if su - oracle -c 'echo $ORACLE_HOME' | grep -q "/app/oracle/product/19/db_1"; then
...
show_install_status() {
    ...
    local files_to_check=(
        "/app/oracle/product/19/db_1/runInstaller"
        "/home/oracle/installdb.rsp"
        "/app/oraInventory/orainstRoot.sh"
        "/app/oracle/product/19/db_1/root.sh"
    )
